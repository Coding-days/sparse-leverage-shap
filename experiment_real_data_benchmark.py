...
"""
Real-data accuracy benchmark across 8 datasets from `shap.datasets`.
Sweeps the sample budget m from the data-poor regime (m < n) through the
data-rich regime (m >> n), and replicates the plotting style of Figure 1
and Figure 3 from "Provably Accurate Shapley Value Estimation via Leverage
Score Sampling" (Musco & Witter, ICLR 2025), adding Sparse Leverage SHAP
as a fourth baseline. Primary metric: relative l2 error against Tree SHAP
ground truth.

Companion experiment: `experiment_support_recovery.py` tests the
O(s log n) sample-complexity claim directly on a synthetic sparse game
with a known active set.
